...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	must(container.Provide(repository.NewGraphCommunityRepository))
	must(container.Provide(repository.NewMessageSuggestionRepository))
	must(container.Provide(repository.NewParseReviewRepository))
	...
	// SessionPrecipitationService must register before MemoryService — the
	// memory worker calls it to precipitate high-value sessions (4.4).
	must(container.Provide(service.NewSessionPrecipitationService))
	...
	must(container.Invoke(chatpipeline.NewPluginRouteRetrieval))
	must(container.Invoke(chatpipeline.NewPluginQueryEnhance))
	...
	must(container.Provide(handler.NewParseReviewHandler))
