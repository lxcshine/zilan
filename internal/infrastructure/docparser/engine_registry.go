...
// ---------------------------------------------------------------------------
// marker — Go-native, calls a self-hosted Marker service
// ---------------------------------------------------------------------------

type markerEngine struct{}

func (e *markerEngine) Name() string { return "marker" }
func (e *markerEngine) Description() string {
	return "Marker self-hosted service (academic PDFs, tables, equations)"
}
func (e *markerEngine) FileTypes(_ bool) []string {
	return []string{"pdf", "docx", "doc", "pptx", "ppt"}
}
func (e *markerEngine) CheckAvailable(_ bool, overrides map[string]string) (bool, string) {
	endpoint := strings.TrimSpace(overrides["marker_endpoint"])
	if endpoint == "" {
		return false, "Marker service not configured"
	}
	return PingMarker(endpoint)
}

// ---------------------------------------------------------------------------
// docling — Go-native, calls a self-hosted docling-serve instance
// ---------------------------------------------------------------------------
...
func (e *doclingEngine) Description() string {
	return "Docling-serve self-hosted service (TableFormer table structure)"
}
func (e *doclingEngine) FileTypes(_ bool) []string {
	return []string{"pdf", "docx", "pptx", "html", "htm"}
}
func (e *doclingEngine) CheckAvailable(_ bool, overrides map[string]string) (bool, string) {
	endpoint := strings.TrimSpace(overrides["docling_endpoint"])
	if endpoint == "" {
		return false, "Docling service not configured"
	}
	return PingDocling(endpoint)
}
