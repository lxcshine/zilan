...
type PipelineState struct {
	...
	// MemoryContext is the rendered long-term-memory block recalled by the
	// MEMORY_RECALL stage (L2 session summaries + L3 facts). Injected into
	// the system prompt by prepareMessagesWithHistory. Empty when memory is
	// disabled or nothing scored high enough.
	MemoryContext string `json:"-"`

	// --- ima-grade retrieval state ---

	// Plan is the per-request retrieval strategy resolved by ROUTE_RETRIEVAL.
	// Nil means "use legacy behavior" (request-level params as-is).
	Plan *RetrievalPlan `json:"-"`
	// EnhancedQueries holds LLM/rule-generated semantic variants of the query
	// (multi-query expansion + step-back abstractions), searched in parallel
	// and fused through the multi-channel RRF.
	EnhancedQueries []string `json:"-"`
	// HydeDocument is the hypothetical answer passage generated for HyDE;
	// it is embedded (not keyword-matched) in the search stage.
	HydeDocument string `json:"-"`
	// StepBackQuery is the abstracted "step-back" query generated for broad
	// conceptual recall. Kept separate from EnhancedQueries so the fusion
	// layer can attribute its channel (ChannelStepBack) correctly.
	StepBackQuery string `json:"-"`
	// ChannelResults holds per-channel ranked lists before fusion. The
	// CHUNK_SEARCH_PARALLEL stage fills it; channel keys are Channel* consts.
	ChannelResults map[string][]*SearchResult `json:"-"`
	// RetrievalFailure is set when the pipeline produced no usable context
	// (fallback path). The completion stage reports it to the feedback loop.
	RetrievalFailure bool `json:"-"`
	// LLMRetrievalIntent is the fine-grained retrieval intent hint emitted by
	// the query-understanding LLM call (raw string; validated against
	// RetrievalIntent before use).
	LLMRetrievalIntent string `json:"-"`
...
func (c *ChatManage) Clone() *ChatManage {
	...
	return &ChatManage{
		...
		PipelineState: PipelineState{
			...
			Plan:                 c.Plan,
			EnhancedQueries:      append([]string(nil), c.EnhancedQueries...),
			HydeDocument:         c.HydeDocument,
			StepBackQuery:        c.StepBackQuery,
...
const (
	...
	ROUTE_RETRIEVAL        EventType = "route_retrieval"
	QUERY_ENHANCE          EventType = "query_enhance"
