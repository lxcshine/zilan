...
type SessionService interface {
	...
	// RecordRetrievalFailure applies the retrieval-feedback update (bounded
	// Hedge-style threshold/top-k/channel-weight adjustment) to the tenant's
	// retrieval configuration after a turn where retrieval failed — either
	// the pipeline produced no usable context (hard failure, channelResults
	// carries the per-channel ranked lists) or the LLM explicitly refused to
	// answer from the retrieved context (soft failure, channelResults nil).
	// The update is persisted to the tenant record. Errors are logged, not
	// returned: feedback must never break the QA path.
	RecordRetrievalFailure(ctx context.Context, intent string, channelResults map[string][]*types.SearchResult)
}
