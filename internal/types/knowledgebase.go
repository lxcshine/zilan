...
type KnowledgeBase struct {
	...
	// DocumentClass is the auto-detected corpus class of this knowledge base
	// ("paper" / "manual" / "faq" / "regulation" / "general"), profiled from
	// chunk-level statistics (heading density, table ratio, sentence length).
	// Empty means unprofiled; the ROUTE_RETRIEVAL stage profiles lazily and
	// persists the result here. Used to apply per-class retrieval presets.
	DocumentClass string `yaml:"document_class" json:"document_class,omitempty" gorm:"column:document_class;type:varchar(32);default:''"`
