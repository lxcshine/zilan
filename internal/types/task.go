...
var queueDefinitions = []QueueDefinition{
	...
	{Name: QueuePostProcess, Pool: WorkerPoolPostProcess, Weight: 1, TaskTypes: []string{
		TypeKnowledgePostProcess, TypeMemoryExtract, TypeSessionPrecipitate,
	}},
	...
	{Name: QueueGraph, Pool: WorkerPoolEnrichment, Weight: 1, SharedWeight: 1, TaskTypes: []string{TypeChunkExtract, TypeGraphCommunityBuild}},
...
const (
	...
	TypeMemoryExtract            = "memory:extract"             // 记忆提取任务（事实三元组/待办/反馈 + L2 会话摘要）
	TypeSessionPrecipitate       = "session:precipitate"        // 高价值会话沉淀任务（自动摘要入库，来源标记 session）
	TypeGraphCommunityBuild      = "graph:community_build"      // GraphRAG 社区摘要重建任务（KB 级，防抖）
)
