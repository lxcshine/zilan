...
func (s *knowledgeService) convert(
	...
	// §5.1 auto-routing: when no explicit engine rule matches a PDF, pick
	// the engine from document characteristics (size/complexity) and the
	// engines the tenant has configured.
	fallbackChain := []string(nil)
	if parserEngine == "" && !isURL {
		available := s.listAvailableParserEngines(ctx, mergedOverrides)
		decision := docparser.AutoRoute(fileType, int64(len(req.FileContent)), mergedOverrides, available)
		parserEngine = decision.Engine
		fallbackChain = decision.FallbackChain
		req.ParserEngine = parserEngine
		logger.Infof(ctx, "[convert] auto-route decision: engine=%q reason=%s fallback=%v",
			decision.Engine, decision.Reason, decision.FallbackChain)
	}
	...
	if result.Error != "" {
		...
		// §5.4: the asynq retry loop exhausted its attempts — the document
		// failed repeatedly and goes to the manual review queue.
		s.enqueueParseReview(ctx, kb, knowledge, payload.FileName, 0,
			"parser error: "+result.Error, parserEngine, "")
		return nil, nil
	}
	...
	// Quality scoring (§5.4): when the builtin engine produced low-quality
	// output, retry once with a heavier engine from the fallback chain and
	// keep the better result. If quality stays low (or parsing failed twice),
	// the document enters the manual review queue.
	qScore, qRetry, qReason := docparser.ParseQualityFromMetadata(result.Metadata)
	retriedWith := ""
	if qRetry && qScore < 0.70 && len(fallbackChain) > 0 {
		nextEngine, ok := docparser.ShouldRetryWithHeavierEngine(parserEngine, fallbackChain, qScore)
		if ok {
			logger.Warnf(ctx, "[convert] low parse quality for %s (score=%.2f reason=%s): retrying with engine=%q",
				knowledge.ID, qScore, qReason, nextEngine)
			if retryReader := s.resolveDocReader(ctx, nextEngine, fileType, isURL, mergedOverrides); retryReader != nil {
				retryReq := *req
				retryReq.ParserEngine = nextEngine
				// §5.4 OCR precision bump on retry: the heavier engine gets a
				// higher render DPI so fine print / formulas are OCR'd sharper.
				retryOverrides := make(map[string]string, len(mergedOverrides)+2)
				for k, v := range mergedOverrides {
					retryOverrides[k] = v
				}
				retryOverrides["pdf_render_dpi"] = "300"
				retryOverrides["ocr_precision"] = "high"
				retryReq.ParserEngineOverrides = retryOverrides
				if retryResult, rerr := s.callDocReaderWithTimeout(ctx, retryReader, &retryReq); rerr == nil && retryResult.Error == "" {
					rScore, _, _ := docparser.ParseQualityFromMetadata(retryResult.Metadata)
					// Adoption rule: a retry is accepted when it either scored
					// better, OR (heavy engines emit no quality metadata, so
					// ParseQualityFromMetadata defaults score to 1.0) it
					// returned strictly more content — either is an improvement.
					if rScore > qScore || len(retryResult.MarkdownContent) > len(result.MarkdownContent) {
						logger.Infof(ctx, "[convert] retry engine=%q improved quality %.2f -> %.2f (chars %d -> %d), adopting retry result",
							nextEngine, qScore, rScore, len(result.MarkdownContent), len(retryResult.MarkdownContent))
						result = retryResult
						// Heavy engines don't emit quality metadata; rScore
						// defaults to 1.0. Clamp to a reasonable "good" value
						// so the doc is not flagged for review after a
						// successful heavy-engine retry.
						qScore = rScore
						retriedWith = nextEngine
					} else {
						logger.Infof(ctx, "[convert] retry engine=%q did not improve (score=%.2f, chars=%d), keeping original",
							nextEngine, rScore, len(retryResult.MarkdownContent))
					}
				}
			}
		}
	...
	if qScore < 0.70 {
		logger.Warnf(ctx, "[convert] low parse quality for %s: score=%.2f retry=%v reason=%s",
			knowledge.ID, qScore, qRetry, qReason)
		docOutput["quality_score"] = qScore
		docOutput["quality_retry_reason"] = qReason
		if retriedWith != "" {
			docOutput["quality_retry_engine"] = retriedWith
		}
		// §5.4 manual review queue: quality still below threshold after the
		// engine retry — surface it to operators.
		engineUsed := parserEngine
		if retriedWith != "" {
			engineUsed = parserEngine + "+" + retriedWith
		}
		s.enqueueParseReview(ctx, kb, knowledge, payload.FileName, qScore, qReason,
			engineUsed, result.Metadata["doc_type"])
	}
	if pages := result.Metadata["pages"]; pages != "" {
		docOutput["pages"] = pages
	}
...
func (s *knowledgeService) resolveDocReader(ctx context.Context, engine, fileType string, isURL bool, overrides map[string]string) interfaces.DocReader {
	...
	case "marker":
		return docparser.NewMarkerReader(overrides)
	case "docling":
		return docparser.NewDoclingReader(overrides)
	case "builtin":
...
// listAvailableParserEngines returns the parser engines usable by this
// deployment (local registry + remote docreader engines), for auto-routing.
func (s *knowledgeService) listAvailableParserEngines(ctx context.Context, overrides map[string]string) []types.ParserEngineInfo {
	docreaderConnected := false
	var remote []types.ParserEngineInfo
	if s.documentReader != nil {
		docreaderConnected = s.documentReader.IsConnected()
		if engines, err := s.documentReader.ListEngines(ctx, overrides); err == nil {
			remote = engines
		}
	}
	return docparser.ListAllEngines(docreaderConnected, overrides, remote)
}

// enqueueParseReview puts a document into the manual review queue (§5.4).
// Best-effort: review-queue failures never break the parse pipeline.
func (s *knowledgeService) enqueueParseReview(
	ctx context.Context,
	kb *types.KnowledgeBase,
	knowledge *types.Knowledge,
	fileName string,
	...
	if s.parseReviewRepo == nil || knowledge == nil {
		return
	}
	...
	if err := s.parseReviewRepo.CreateReviewItem(ctx, item); err != nil {
		logger.Warnf(ctx, "[convert] failed to enqueue parse review for %s: %v", knowledge.ID, err)
	} else {
		logger.Infof(ctx, "[convert] enqueued parse review %s for knowledge %s (score=%.2f)", item.ID, knowledge.ID, qualityScore)
	}
}

func kbIDOf(kb *types.KnowledgeBase) string {
	if kb == nil {
		return ""
	}
	return kb.ID
