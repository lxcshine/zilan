...
func (s *sessionService) KnowledgeQA(
	...
	// Memory recall is independent of the session-history window: long-term
	// facts and cross-session summaries matter even in single-turn sessions.
	memoryEnabled := s.memoryService != nil &&
		s.memoryService.IsEnabled(ctx, types.SessionOwnerIDFromContext(ctx))
	...
	if !needsRAG {
		...
		pipeline = types.NewPipelineBuilder().
			...
			AddIf(memoryEnabled, types.MEMORY_RECALL).
		...
		pipeline = types.NewPipelineBuilder().
			...
			Add(types.ROUTE_RETRIEVAL).
			Add(types.QUERY_ENHANCE).
...
func (s *sessionService) handleFallbackResponse(ctx context.Context, chatManage *types.ChatManage) {
	// Retrieval produced no usable context: mark the turn as a hard retrieval
	// failure and let the feedback loop broaden future recall parameters.
	chatManage.RetrievalFailure = true
	intent := ""
	if chatManage.Plan != nil {
		intent = string(chatManage.Plan.Intent)
	}
	s.RecordRetrievalFailure(ctx, intent, chatManage.ChannelResults)
...
// retrievalFeedbackMu serializes tenant retrieval-config read-modify-write
// cycles triggered by the feedback loop. Failures are rare, so a process-wide
// mutex is sufficient (cross-instance races are bounded by the Hedge update's
// small step size).
var retrievalFeedbackMu sync.Mutex

// RecordRetrievalFailure implements interfaces.SessionService. It applies the
// bounded Hedge-style retrieval-parameter update to the tenant's persisted
// retrieval configuration. Best-effort: all errors are logged, never returned.
func (s *sessionService) RecordRetrievalFailure(
	ctx context.Context,
	intent string,
	...
	tenantID, ok := types.TenantIDFromContext(ctx)
	if !ok || tenantID == 0 {
		logger.Warnf(ctx, "Retrieval feedback skipped: tenant ID not found in context")
		return
	}

	retrievalFeedbackMu.Lock()
	defer retrievalFeedbackMu.Unlock()

	tenant, err := s.tenantService.GetTenantByID(ctx, tenantID)
	if err != nil || tenant == nil {
		logger.Warnf(ctx, "Retrieval feedback skipped: failed to load tenant %d: %v", tenantID, err)
		return
	}

	updated := chatpipeline.ApplyRetrievalFeedback(tenant.RetrievalConfig, intent, channelResults)
	tenant.RetrievalConfig = updated
	if _, err := s.tenantService.UpdateTenant(ctx, tenant); err != nil {
		logger.Errorf(ctx, "Failed to persist retrieval feedback for tenant %d: %v", tenantID, err)
		return
	}
	logger.Infof(ctx,
		"Retrieval feedback applied for tenant %d (intent=%s): vector_threshold=%.4f keyword_threshold=%.4f embedding_top_k=%d",
		tenantID, intent, updated.VectorThreshold, updated.KeywordThreshold, updated.EmbeddingTopK)
}
