...
func (s *ChunkExtractService) Handle(ctx context.Context, t *asynq.Task) error {
	...
	// Debounced KB-scoped community rebuild (GraphRAG): the Unique TTL in
	// MaybeEnqueueBuild collapses the per-chunk burst into one rebuild.
	if s.communityService != nil {
		s.communityService.MaybeEnqueueBuild(ctx, p.TenantID, chunk.KnowledgeBaseID, "ingest")
	}
