...
func (s *knowledgeBaseService) ProcessKBDelete(ctx context.Context, t *asynq.Task) error {
	...
	if len(knowledgeList) > 0 {
		...
		// Delete GraphRAG community summaries of this KB
		if s.communityRepo != nil {
			if err := s.communityRepo.DeleteByKnowledgeBase(ctx, tenantID, kbID); err != nil {
				logger.Warnf(ctx, "Failed to delete graph communities for KB %s: %v", kbID, err)
			}
		}
