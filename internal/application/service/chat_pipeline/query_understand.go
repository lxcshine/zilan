...
type queryUnderstandOutput struct {
	...
	// RetrievalIntent is the optional fine-grained retrieval strategy hint
	// (factual/summary/comparison/reasoning/exploratory). Validated against
	// types.RetrievalIntent by the ROUTE_RETRIEVAL stage before use.
...
func (p *PluginQueryUnderstand) parseOutput(chatManage *types.ChatManage, raw string) {
	...
	if output, ok := parseStructuredQueryOutput(content); ok {
		...
		chatManage.LLMRetrievalIntent = strings.TrimSpace(output.RetrievalIntent)
		return
	}
...
func parseStructuredQueryOutputJSON(content string) (queryUnderstandOutput, bool) {
	...
	out.RetrievalIntent = strings.TrimSpace(firstStringField(obj,
		"retrieval_intent", "retrieval_strategy", "search_intent"))
