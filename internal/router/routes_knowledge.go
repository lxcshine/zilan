...
func RegisterKnowledgeBaseRoutes(r *gin.RouterGroup, handler *handler.KnowledgeBaseHandler, g *rbacGuards) {
	...
	kbManagement := kb.With(apiKeyManageKnowledgeBases(apiKeyFullAccess()))
	{
		...
		// GraphRAG 社区摘要：查看 — Viewer+ 读权限；重建 — 与 KB 内容写入同档（Contributor + 写权限）
		kb.GET("/:id/graph/communities", g.Viewer(), g.KBAccessRead("id"), handler.ListGraphCommunities)
		kb.POST("/:id/graph/communities/rebuild", g.Contributor(), g.KBAccessWrite("id"), handler.RebuildGraphCommunities)
	}
}
