...
type AsynqTaskParams struct {
	...
	GraphCommunity       interfaces.GraphCommunityService
...
func RunAsynqServer(params AsynqTaskParams) *asynq.ServeMux {
	...
	// Register memory extraction handler (L3 facts + L2 session summary)
	mux.HandleFunc(types.TypeMemoryExtract, params.MemoryService.ProcessMemoryExtract)

	// Register session precipitation handler (高价值会话自动摘要入库, 4.4)
	mux.HandleFunc(types.TypeSessionPrecipitate, params.PrecipitationService.ProcessSessionPrecipitate)

	// Register GraphRAG community summary rebuild handler (KB 级，防抖)
	mux.HandleFunc(types.TypeGraphCommunityBuild, params.GraphCommunity.ProcessGraphCommunityBuild)
