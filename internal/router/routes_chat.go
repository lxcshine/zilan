...
func RegisterSessionRoutes(
	...
	sessions := g.apiKeyGroup(r.Group("/sessions", g.Viewer()), apiKeyChat(apiKeyFullAccess()))
	{
		...
		// 知识沉淀 (4.4): one-click "将本次对话整理为 Wiki". Idempotent per
		// session — repeated calls refresh the session/<id> page.
		sessions.POST("/:session_id/wiki", handler.CreateWikiFromSession)
