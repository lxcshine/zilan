...
// RegisterMemoryRoutes wires the per-user long-term memory management
// surface under /memory. The v1 group already applies middleware.Auth; the
// service scopes every operation to the caller's (tenant, user) pair from
// the request context, so no additional role gate is needed — a user can
// only ever see and modify their own memories (GDPR self-service).
//
// memoryHandler may be nil in environments built without the memory
// dependency wired; that's a no-op registration which is preferable to a
// startup crash.
func RegisterMemoryRoutes(r *gin.RouterGroup, memoryHandler *handler.MemoryHandler) {
	...
	memory := r.Group("/memory")
	{
		...
		memory.DELETE("", memoryHandler.DeleteAllMemories)
	}
}

// RegisterParseReviewRoutes wires the parse quality review queue endpoints
// (§5.4). Tenant-authenticated; the handler extracts the tenant from the
// context. Returns silently if the handler is nil (e.g. dependency not wired).
func RegisterParseReviewRoutes(r *gin.RouterGroup, reviewHandler *handler.ParseReviewHandler) {
