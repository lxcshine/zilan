...
type Handler struct {
	...
	precipitationService interfaces.SessionPrecipitationService // Service for knowledge precipitation (session → KB doc / wiki)
...
func NewHandler(
	...
	memoryService interfaces.MemoryService,
	precipitationService interfaces.SessionPrecipitationService,
...
func (h *Handler) DeleteSession(c *gin.Context) {
	...
	// Drop the L2 short-term memory bound to this session (best-effort; L3
	// long-term facts intentionally survive session deletion).
	if h.memoryService != nil {
		if tenantID, ok := types.TenantIDFromContext(ctx); ok {
			h.memoryService.DeleteSessionMemory(ctx, tenantID, id)
		}
	}
...
func (h *Handler) ClearSessionMessages(c *gin.Context) {
	...
	// The rolling L2 summary describes the cleared conversation; drop it.
	if h.memoryService != nil {
		if tenantID, ok := types.TenantIDFromContext(ctx); ok {
			h.memoryService.DeleteSessionMemory(ctx, tenantID, id)
		}
...
func (h *Handler) BatchDeleteSessions(c *gin.Context) {
	...
	if req.DeleteAll {
		...
		// Drop every L2 short-term memory of the caller (best-effort).
		if h.memoryService != nil {
			if _, err := h.memoryService.DeleteAllSummariesForUser(ctx); err != nil {
				logger.Warnf(ctx, "delete all session memories failed: %v", err)
			}
		}
		c.JSON(http.StatusOK, gin.H{
			"success": true,
	...
	// Drop L2 short-term memories bound to the deleted sessions (best-effort).
	if h.memoryService != nil {
		if tenantID, ok := types.TenantIDFromContext(ctx); ok {
