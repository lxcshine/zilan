...
# (extracted without sudo via apt-get download + dpkg-deb -x).
SQLITE_SHIM_DIR := $(strip $(wildcard /tmp/sqlite_shim))
SQLITE_CFLAGS := $(if $(SQLITE_SHIM_DIR),-I$(SQLITE_SHIM_DIR)/usr/include)
SQLITE_LDFLAGS := $(if $(SQLITE_SHIM_DIR),-L$(SQLITE_SHIM_DIR)/usr/lib)
...
build-prod:
	...
	CGO_ENABLED=1 CGO_CFLAGS="-Wno-deprecated-declarations $(SQLITE_CFLAGS)" CGO_LDFLAGS="$(SQLITE_LDFLAGS)" go build -ldflags="-w -s $$LDFLAGS" -o $(BINARY_NAME) $(MAIN_PATH)
...
build-lite:
	...
	CGO_ENABLED=1 CGO_CFLAGS="-Wno-deprecated-declarations $(SQLITE_CFLAGS)" CGO_LDFLAGS="$(SQLITE_LDFLAGS)" go build -tags "sqlite_fts5" -ldflags="-w -s $$LDFLAGS" -o $(BINARY_NAME)-lite $(MAIN_PATH)
