 ...
 *
 * Storage layout: Zilan_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (Zilan_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('zilan_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[Zilan] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `Zilan_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `Zilan_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `Zilan_anon_${suffix}`
    const legacyKey = `Zilan_${suffix}`
