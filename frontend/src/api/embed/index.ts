...
export const EMBED_CHAT_SESSION_STORAGE_PREFIX = 'zilan-embed-session:'

/** localStorage key prefix for anonymous embed visitor ids (per channel). */
export const EMBED_VISITOR_STORAGE_PREFIX = 'zilan-embed-visitor:'
...
const EMBED_MSG_SOURCE = 'zilan-embed'
const EMBED_HOST_SOURCE = 'zilan-host'
...
export function buildWidgetSnippet(
  ...
  const attrs = [
    `src="${escapeHtmlAttr(`${base}/zilan-widget.js`)}"`,
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER = 'https://your-backend.example.com/zilan/embed-token'
...
export function buildSecureServerNodeExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  return [
    `// Node/Express — keep ZILAN_PUBLISH_TOKEN only on the server (env var).`,
    `app.get('/zilan/embed-token', async (req, res) => {`,
    ...
    `    method: 'POST',`,
    `    headers: {`,
    `      Authorization: 'Embed ' + process.env.ZILAN_PUBLISH_TOKEN,`,
...
export function buildSecureServerGoExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  const exchangeUrl = `${base}/api/v1/embed/${channelId}/exchange`
  return [
    `// Go net/http — keep ZILAN_PUBLISH_TOKEN only on the server (env var).`,
    ...
    `  req.Header.Set("Authorization", "Embed "+os.Getenv("ZILAN_PUBLISH_TOKEN"))`,
