...
const protectedFileCacheState = (() => {
  ...
  const scope = window as typeof window & {
    __zilanProtectedFileCacheV1__?: ProtectedFileCacheState;
  };
  scope.__zilanProtectedFileCacheV1__ ||= fresh();
  return scope.__zilanProtectedFileCacheV1__;
})();
...
function getProtectedFileRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('zilan_token') || '').trim();
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    const selectedTenantId = (localStorage.getItem('zilan_selected_tenant_id') || '').trim();
    if (selectedTenantId) {
      ...
      // active tenant into zilan_tenant, leaving authenticated file
