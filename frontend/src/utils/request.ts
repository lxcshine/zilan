...
instance.interceptors.request.use(
  (config) => {
    ...
    if (!isEmbedAuth) {
      const token = localStorage.getItem('zilan_token');
      if (token) {
    ...
    // 但这条优化会被任何把 zilan_tenant 写成激活空间的代码（OIDC
    ...
    if (!isEmbedAuth && !isEmbedPath) {
      const selectedTenantId = localStorage.getItem('zilan_selected_tenant_id');
      if (selectedTenantId) {
...
instance.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (error.response.status === 401 && !originalRequest._retry && !originalRequest.url?.includes('/auth/refresh')) {
      ...
      const refreshToken = localStorage.getItem('zilan_refresh_token');
      ...
      if (refreshToken) {
        try {
          ...
          if (response.success && response.data) {
            ...
            localStorage.setItem('zilan_token', token);
            localStorage.setItem('zilan_refresh_token', newRefreshToken);
