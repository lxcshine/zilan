    ...
    <div class="aside_box" :class="{ 'aside_box--collapsed': uiStore.sidebarCollapsed }">
        ...
        <div class="logo_row" v-if="!uiStore.sidebarCollapsed">
            <div class="logo_box" @click="router.push('/platform/knowledge-bases')" style="cursor: pointer;">
                <div class="logo_mark" aria-hidden="true">知</div>
                <span class="logo_word">知澜</span>
...
const isMenuItemActive = (itemPath: string): boolean => {
    ...
    switch (itemPath) {
        ...
        case 'memory':
            return currentRoute === 'memoryList';
        case 'organizations':
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'memory' || item.path === 'organizations' || item.path === 'creatChat'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'memory' || item.path === 'organizations' || item.path === 'creatChat') {
            return false;
        }
        return true;
    });
...
const getIcon = (path: string) => {
    ...
    const memoryActiveState = route.name === 'memoryList';
...
.aside_box {
    ...
    .menu_item_active {
        ...
        // active 图标是绿色 SVG（<img> 加载无法跟随 token），滤镜压为深色以匹配单色品牌
        .menu_icon img.icon {
...
// Active icons: desaturate the green SVG and brighten to match the light brand tone in dark mode
