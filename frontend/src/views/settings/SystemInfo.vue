        ...
        <t-alert theme="error" :title="$t('system.dbMigrationFailedTitle')" style="width: 100%;">
          <template #default>
            ...
            <div class="migration-error-actions">
              ...
              <span v-if="troubleshootingDocsURL && issueTrackerBase" class="migration-error-actions-sep">·</span>
              <t-link
                v-if="issueTrackerBase"
                theme="primary"
                :href="reportIssueURL"
                target="_blank"
                rel="noopener noreferrer"
...
const troubleshootingDocsURL =
  (import.meta.env.VITE_TROUBLESHOOTING_DOC_URL as string) || ''

const issueTrackerBase = (import.meta.env.VITE_ISSUE_TRACKER_URL as string) || ''
...
const reportIssueURL = computed(() => {
  ...
  if (errMsg) {
    const body = [
      '### Environment',
      `- Zilan version: ${systemInfo.value?.version || 'unknown'}`,
  ...
  return `${issueTrackerBase}?${params.toString()}`
})
