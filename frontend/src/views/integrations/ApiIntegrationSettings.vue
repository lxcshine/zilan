        ...
        <div class="row row--doc">
          <div class="row-info">
            ...
            <p>
              ...
              <a v-if="apiDocUrl" class="doc-link" @click="openApiDoc">
...
// API 文档外链：未配置则隐藏入口
const apiDocUrl = (import.meta.env.VITE_API_DOC_URL as string) || ''

type WeKnoraDesktopWindow = Window & {
  __ZILAN_API_BASE__?: string
  __ZILAN_API_LAN_BASE__?: string
  go?: {
    main?: {
      App?: {
...
// Send on each Zilan API request:
...
async function tryLoadWailsApiBaseURL() {
  ...
  for (let i = 0; i < 40; i++) {
    const injected = win.__ZILAN_API_BASE__
...
async function tryLoadWailsLanHints(win: WeKnoraDesktopWindow) {
  const injectedLan = win.__ZILAN_API_LAN_BASE__
...
function openApiDoc() {
  if (apiDocUrl) window.open(apiDocUrl, '_blank')
}
