...
export const useAuthStore = defineStore('auth', () => {
  ...
  //
  // SECURITY: This value is read from localStorage (`zilan_memberships`)
  ...
  const setUser = (userData: UserInfo) => {
    ...
    user.value = userData
    // 保存到localStorage
    localStorage.setItem('zilan_user', JSON.stringify(userData))
  ...
  const setTenant = (tenantData: TenantInfo | null) => {
    ...
    if (tenantData) {
      localStorage.setItem('zilan_tenant', JSON.stringify(tenantData))
    } else {
      localStorage.removeItem('zilan_tenant')
  ...
  const setToken = (tokenValue: string) => {
    ...
    localStorage.setItem('zilan_token', tokenValue)
  }

  const setRefreshToken = (refreshTokenValue: string) => {
    refreshToken.value = refreshTokenValue
    localStorage.setItem('zilan_refresh_token', refreshTokenValue)
  }

  const setKnowledgeBases = (kbList: KnowledgeBaseInfo[]) => {
    // 确保输入是数组
    knowledgeBases.value = Array.isArray(kbList) ? kbList : []
    localStorage.setItem('zilan_knowledge_bases', JSON.stringify(knowledgeBases.value))
  }

  const setCurrentKnowledgeBase = (kb: KnowledgeBaseInfo | null) => {
    currentKnowledgeBase.value = kb
    if (kb) {
      localStorage.setItem('zilan_current_kb', JSON.stringify(kb))
    } else {
      localStorage.removeItem('zilan_current_kb')
    }
  }
  ...
  const clearTenantScopedClientState = () => {
    try {
      localStorage.removeItem('zilan_last_chat_model_id')
      localStorage.removeItem('zilan_current_kb')
      const raw = localStorage.getItem('Zilan_settings')
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          localStorage.setItem('Zilan_settings', JSON.stringify(parsed))
        }
      }
    } catch (e) {
  ...
  const setSelectedTenant = (tenantId: number | null, tenantName: string | null = null) => {
    ...
    if (tenantId !== null) {
      localStorage.setItem('zilan_selected_tenant_id', String(tenantId))
      if (tenantName) {
        localStorage.setItem('zilan_selected_tenant_name', tenantName)
      }
    } else {
      localStorage.removeItem('zilan_selected_tenant_id')
      localStorage.removeItem('zilan_selected_tenant_name')
    }
    if (tenantChanged) {
  ...
  const setMemberships = (
    ...
    localStorage.setItem('zilan_memberships', JSON.stringify(memberships.value))
  }
  ...
  const setLiteMode = (value: boolean) => {
    ...
    if (value) {
      localStorage.setItem('zilan_lite_mode', 'true')
    } else {
      localStorage.removeItem('zilan_lite_mode')
    }
  }
  ...
  const logout = () => {
    ...
    // 清空localStorage
    localStorage.removeItem('zilan_user')
    localStorage.removeItem('zilan_tenant')
    localStorage.removeItem('zilan_token')
    localStorage.removeItem('zilan_refresh_token')
    localStorage.removeItem('zilan_knowledge_bases')
    localStorage.removeItem('zilan_current_kb')
    localStorage.removeItem('zilan_selected_tenant_id')
    localStorage.removeItem('zilan_selected_tenant_name')
    localStorage.removeItem('zilan_memberships')
    localStorage.removeItem('zilan_lite_mode')
    isLiteMode.value = false
    try {
      sessionStorage.removeItem('zilan_lite_last_path')
    } catch {
      /* ignore */
    }
    reloadUserPreferences()
  ...
  const initFromStorage = () => {
    // 从localStorage恢复状态
    const storedUser = localStorage.getItem('zilan_user')
    const storedTenant = localStorage.getItem('zilan_tenant')
    const storedToken = localStorage.getItem('zilan_token')
    const storedRefreshToken = localStorage.getItem('zilan_refresh_token')
    const storedKnowledgeBases = localStorage.getItem('zilan_knowledge_bases')
    const storedCurrentKb = localStorage.getItem('zilan_current_kb')
    const storedSelectedTenantId = localStorage.getItem('zilan_selected_tenant_id')
    const storedSelectedTenantName = localStorage.getItem('zilan_selected_tenant_name')
    ...
    const storedMemberships = localStorage.getItem('zilan_memberships')
    ...
    isLiteMode.value = localStorage.getItem('zilan_lite_mode') === 'true'
  }
