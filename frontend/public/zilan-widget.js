/**
 * Zilan embed widget SDK — floating chat launcher.
 *
 * Programmatic:
 *   Zilan.init({ channel, token, position, primaryColor, title, baseUrl })
 *   Zilan.open() | close() | toggle() | destroy()
 *   Zilan.on('ready', fn) | off('ready', fn)
 *
 * Host context & actions (postMessage to iframe):
 *   Zilan.setContext({ userId, page, ... })
 *     Inject visitor/page context merged into each chat query.
 *   Zilan.openWithQuery('How do I reset my password?')
 *     Opens the panel (if closed) and sends the query when the iframe is ready.
 *   Zilan.setLocale('en-US')
...
(function (global) {
  ...
  var HOST_SOURCE = 'zilan-host';
  var EMBED_SOURCE = 'zilan-embed';
  ...
  function emit(event, payload) {
    ...
    handlers.slice().forEach(function (fn) {
      try { fn(payload); } catch (e) { console.error('[Zilan]', e); }
    });
  }
  ...
  function createWidget(opts) {
    ...
    if (!channelId || (!staticToken && !tokenEndpoint)) {
      console.warn('[Zilan] channel and (token or tokenEndpoint) are required');
      return null;
    }
    ...
    function loadToken(force) {
      ...
      tokenInFlight = fetch(tokenEndpoint, {
        ...
        .catch(function (e) {
          console.error('[Zilan] failed to load token', e);
          throw e;
        })
    ...
    if (!baseUrl) {
      ...
      if (script && script.src) {
        baseUrl = script.src.replace(/\/zilan-widget\.js.*$/, '');
      } else {
    ...
    function postHostPayload(type, payload) {
      if (!iframe.contentWindow) {
        console.warn('[Zilan] iframe not ready');
        return false;
      }
    ...
    function setContext(ctx) {
      if (!ctx || typeof ctx !== 'object') {
        console.warn('[Zilan] setContext expects an object');
        return;
      }
    ...
    function openWithQuery(query) {
      ...
      if (!text) {
        console.warn('[Zilan] openWithQuery requires a non-empty query');
        return;
      }
      setOpen(true);
    ...
    function setLocale(locale) {
      ...
      if (!loc) {
        console.warn('[Zilan] setLocale requires a locale string');
        return;
      }
